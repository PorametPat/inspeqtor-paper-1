...
def plot_history(
    history: list,
    lr_schedule: Union[Callable[[int], float], None] = None,
    window_size: int = 50,
    ...
    axes[0].plot(train_x, train_y, label="Training loss (MSE)", color="#6a82fb", alpha=0.25)
    axes[0].plot(validate_x, validate_y, label="Validation loss (MSE)", color="#fc5c7d", alpha=0.25)
    
    if with_epoch_loss:

        epoch = hist_df[["global_step", "epoch_loss"]].replace(0, jnp.nan).dropna().values
        epoch_x = epoch[:, 0]
        epoch_y = epoch[:, 1]
    ...
    axes[0].plot(
        validate_x, ma_validate_y, label="moving average Validation loss (MSE)", color="#fc5c7d"
    )
